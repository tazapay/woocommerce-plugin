...
class TPAY_Gateway extends WC_Payment_Gateway
{
  ...
  public function __construct()
  {
    ...
    // Adds Icon
    add_filter('woocommerce_gateway_icon', array($this, 'tzp_woocommerce_icons'), 10, 2);
    ...
    add_action('wp_enqueue_scripts', array($this, 'check_if_block_checkout'));
  }
  ...
  public function check_if_block_checkout()
  {
    if (is_checkout()) {
      // Check if the block checkout is being used
      if (has_block('woocommerce/checkout', get_post())) {
        // This is the block-based checkout
        // Your custom code here
        include 'block-checkout-icons.php';
      }
    }
  }

  public function tzp_woocommerce_icons($icon, $id)
  {
    if ($id === 'tazapay') {
  ...
  public function tzp_thankyou_page($order_id)
  {
    global $woocommerce;
